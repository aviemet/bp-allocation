...
interface RoundOrg extends Organization {
	roundFunds?: number
	percent?: number
}
...
class Leverage {
	...
	getLeverageSpreadRounds() {
		...
		while(this.leverageRemaining >= 1 && this._numFullyFundedOrgs() < this.orgs.length && nRounds < 10) {
			...
			let roundOrgs = this.orgs.map(org => {
				...
				trackers.givenThisRound = roundFloat(trackers.givenThisRound + (org.roundFunds ?? 0))
	...
	_orgRoundValues(org: RoundOrg, nRounds: number) {
		/** DEFAULTS **/
	...
	finalRoundAllcoation() {
		...
		let funds = lastRound.orgs.reduce((sum, org) => {
			return sum + (org.leverageFunds > 0 ? (org.roundFunds ?? 0) : 0)
		}, 0)
