...
export const roundFloat = (value: StringConstructor|number, decimal: number = 2) => {
	return parseFloat(parseFloat(String(value)).toFixed(decimal))
}
...
export const paginate = (collection: Record<string, unknown>[], page: number, itemsPerPage: number) => {
	return collection.slice(page * itemsPerPage, (page + 1) * itemsPerPage)
}
...
export const filterCollection = (collection: Record<string, string>[], search: string, fields: string[]) => {
	...
	return collection.filter(member => searchParts.every(word => {
