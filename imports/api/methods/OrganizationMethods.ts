...
const OrganizationMethods = {
	...
	create: new ValidatedMethod({
		...
		run(data: Partial<Organization>) {
			let result = {}
			Organizations.insert(data, (error: Error, response: string) => {
				if(error){
					console.error(error)
				} else {
	...
	update: new ValidatedMethod({
		...
		run({ id, data }: { id: string, data: Partial<Organization> }) {
	...
	remove: new ValidatedMethod({
		...
		run(id: string) {
			const org = Organizations.findOne(id)
			...
			if(org){
				...
				// Remove organization
				return Organizations.remove(id, (err: Error) => {
					if(err) console.error(err)

					Themes.update({ _id: org.theme }, { $pull: { organizations: id } }, (err: Error) => {
						if(err) console.error(err)
					})
				})
			} else {
	...
	pledge: new ValidatedMethod({
		...
		run({ _id: id, amount, member, anonymous }: Partial<MatchPledge>) {
			if(!amount) return
	...
	removePledge: new ValidatedMethod({
		...
		run({ orgId, pledgeId }: { orgId: string, pledgeId: string}) {
	...
	removePledgeById: new ValidatedMethod({
		...
		run({ themeId, pledgeIds }: { themeId: string, pledgeIds: string[] }) {
	...
	topOff: new ValidatedMethod({
		...
		run({ id, negate }: { id: string, negate: boolean }) {
			negate = negate || false
			...
			if(!negate)	{
				topOffAmount = org.ask - org.amountFromVotes - org.pledgeTotal
				// TODO: Used to use org.pledges (which is an array of MatchPledge), was that an error as I suspect?
