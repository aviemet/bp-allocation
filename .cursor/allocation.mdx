...
This hierarchy provides global state management through MobX stores and Meteor subscriptions. Components should use the existing provider hooks (`useData()`, `usePledgeAnimation()`, `useTheme()`, `useSettings()`, etc.) rather than duplicating providers.
...
- Pledge animation state lives in a dedicated PledgeAnimationStore, separate from AppStore for proper separation of concerns
- PledgeAnimationStore provides `replayPledgeAnimation()` method to trigger animations without re-saving to database
- Uses a reactive `shouldStartAnimation` flag to signal when replay is triggered, allowing PledgesOverlay to start animations even when not monitoring new database pledges
- PledgesOverlay automatically animates new pledges and tracks displayed pledges via `displayedPledges` Set
- Admin panel can replay any existing pledge animation via the replay button
- PledgeAnimationProvider sits high in the provider hierarchy (after DataProvider) for global access across admin and presentation views
- Cross-tab/window communication uses a Meteor collection (`PledgeReplays`) with DDP syncing - admin calls `PledgeReplayMethods.triggerPledgeReplay` which inserts server-side, the presentation screen observes it reactively via subscription and triggers the animation
- Server-side Meteor Method prevents direct client inserts for security while maintaining real-time cross-client sync
- This architecture separates animation state from database state, allowing flexible replay without data duplication while leveraging Meteor's reactive data protocol for cross-client communication
...
- **Critical**: Collections must be initialized synchronously when subscriptions become ready, not only in `onReady` callbacks
- Providers check `subscription.ready() && !collection` to initialize collections immediately if subscription is ready before `onReady` fires
- `isLoading` state must check both `subscription.ready()` AND collection existence: `isLoading: !subscription?.ready() || !collection`
