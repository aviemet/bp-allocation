...
export const setMessageSendingFlag = async (theme: Theme, message: Message) => {
...
export const setMessageSentFlag = async (theme: Theme, message: Message) => {
	const data = { messageId: message._id, sending: false, sent: true, error: false }
	await Themes.update({ _id: theme._id, 'messagesStatus.messageId': message._id }, { $set: { 'messagesStatus.$': data } })
}

export const setMessageErrorFlag = async (theme: Theme, message: Message) => {
